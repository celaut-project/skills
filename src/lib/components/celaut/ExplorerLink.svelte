<script lang="ts">
   ...
   * a private node, etc.).
   */
  import { web_explorer_uri_box } from '$lib/common/store';
  ...
  $: isRealBoxId = typeof boxId === 'string' && /^[0-9a-fA-F]{32,}$/.test(boxId);
  $: href = isRealBoxId ? `${$web_explorer_uri_box}${boxId}` : '';
</script>

{#if isRealBoxId}
  <a class="explorer-link" href={href} target="_blank" rel="noopener noreferrer" title={liveTooltip}>
